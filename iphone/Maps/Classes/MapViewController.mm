...
- (void)handlePinch:(UIPinchGestureRecognizer *)recognizer API_AVAILABLE(ios(14.0)) {
  ...
  switch (recognizer.state) {
    ...
    case UIGestureRecognizerStateChanged:
    {
      ...
      const CGFloat delta = scale - currentScale;
...
- (void)handleRotation:(UIRotationGestureRecognizer *)recognizer API_AVAILABLE(ios(14.0)) {
  ...
  switch (recognizer.state) {
    ...
    case UIGestureRecognizerStateChanged:
    {
      ...
      GetFramework().Rotate(currentRotation == 0 ? recognizer.rotation : currentRotation + recognizer.rotation, false);
      break;
    }
    case UIGestureRecognizerStateEnded:
...
- (m2::PointD)getZoomPoint API_AVAILABLE(ios(14.0)) {
  return m2::PointD(self.pointerLocation.x * 2, self.pointerLocation.y * 2);
}
